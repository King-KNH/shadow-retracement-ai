import pandas as pd
import numpy as np
from structure_detection import detect_swings, get_structure_points, determine_bias
...
def detect_setups(h1_df, m1_df, lookback=5, poc_bins=50):
    ...
    for i in range(1, len(legs)):
        ...
        valid_pd = (leg['direction'] == 'bullish' and zone_type == 'discount') or \
                   (leg['direction'] == 'bearish' and zone_type == 'premium')

        # Biais multi-swing (structure réelle de fond, pas juste la leg précédente)
        bias = determine_bias(points, leg['start_time'])
        valid_bias = (leg['direction'] == 'bullish' and bias == 'bullish') or \
                     (leg['direction'] == 'bearish' and bias == 'bearish')
        ...
        setups.append({
            ...
            'zone_type': zone_type, 'valid_pd_filter': valid_pd,
            'bias': bias, 'valid_bias_filter': valid_bias,
            'valid_pd_and_bias': valid_pd and valid_bias,
            'poc': poc,
