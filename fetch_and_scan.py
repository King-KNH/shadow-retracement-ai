...
Secrets requis :
  TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, GEMINI_API_KEY (optionnel)
"""
import os
import json
import requests
import websocket
import pandas as pd
from shadow_retracement_ai import scan_active_setups, format_signal
import state as st
...
def ask_gemini_judgment(asset, sig):
...
Réponds en français, de façon concise."""
    try:
        ...
        r = requests.post(url, json={"contents": [{"parts": [{"text": prompt}]}],
                           "generationConfig": {"maxOutputTokens": 250, "temperature": 0.3}}, timeout=30)
        r.raise_for_status()
        return r.json()["candidates"][0]["content"]["parts"][0]["text"].strip()
    except Exception as e:
        print(f"Avis Gemini indisponible ({e}), notification envoyée sans jugement.")
        return None
    ...
    for asset, symbol in DERIV_SYMBOLS.items():
        try:
            ...
            for sig in signals:
                ...
                if not record["notified_detected"]:
                    text = format_signal(sig, asset)
                    judgment = ask_gemini_judgment(asset, sig)
