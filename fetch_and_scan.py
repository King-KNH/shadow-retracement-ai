...
DERIV_APP_ID = "34uRXNzSHG4xRAmxYIS4b"  # app_id personnel enregistré (remplace le 1089 partagé)
DERIV_WS_URL = f"wss://ws.derivws.com/websockets/v3?app_id={DERIV_APP_ID}"
DERIV_SYMBOLS = {"XAUUSD": "frxXAUUSD", "EURUSD": "frxEURUSD", "BTCUSD": "cryBTCUSD"}
...
def fetch_deriv_since(symbol, granularity, last_epoch):
    ...
    try:
        request = {
            ...
            "count": 5000,  # garde-fou : borne haute, sans influencer la plage start/end
        }
        ws.send(json.dumps(request))
        response = json.loads(ws.recv())
        if "error" in response:
        ...
        print(f"  -> fetch_deriv_since({symbol}, gran={granularity}): {len(candles)} bougie(s) reçue(s) depuis {pd.Timestamp(last_epoch, unit='s')}")
        if not candles:
...
def run():
    ...
    # GARDE-FOU WEEK-END TEMPORAIREMENT DÉSACTIVÉ POUR TEST APP_ID DERIV (à réactiver après)
