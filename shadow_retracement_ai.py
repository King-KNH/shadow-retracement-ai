...
ASSET_CONFIG = {
    'XAUUSD': {'pip_size': 0.01, 'spread_pips': 25, 'sl_slippage_pips': 12,
               'min_rr': 2.0, 'min_stop_pips': 0, 'max_distance_pips': 3000,
               'require_bias_filter': True, 'lookback': 2},
    'EURUSD': {'pip_size': 0.0001, 'spread_pips': 1.2, 'sl_slippage_pips': 0.6,
               'min_rr': 2.5, 'min_stop_pips': 10, 'max_distance_pips': 150,
               'require_bias_filter': False, 'lookback': 2},
    'BTCUSD': {'pip_size': 1.0, 'spread_pips': 25, 'sl_slippage_pips': 40,
               'min_rr': 2.46, 'min_stop_pips': 8.7, 'max_distance_pips': 700,
               'require_bias_filter': True, 'lookback': 2},
}
...
def scan_active_setups(m1_df, asset, h1_df=None):
    ...
    if h1_df is None:
        tf = aggregate(m1_df)
        h1_df = tf['H1']
    setups = detect_setups(h1_df, m1_df, lookback=cfg.get('lookback', 5), poc_bins=50)
    filter_col = 'valid_pd_and_bias' if cfg.get('require_bias_filter') else 'valid_pd_filter'
    valid = setups[setups[filter_col]].copy()
